...
from tensorflow.keras.layers import Input, Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, Reshape, LeakyReLU, BatchNormalization
from tensorflow.keras.models import Model
from tensorflow.keras.regularizers import l2

from yolo3.utils import compose
from yolo3.mobilenetv3_base import return_activation, conv_block, bottleneck
from tensorflow.keras.utils import plot_model




def MobileNetV3_Large(inputs, alpha=1.0, plot=False):
    """

    # Arguments
        inputs: 4D tensor,inputs of model
...
def MobileNetV3_Small(inputs, alpha=1.0, plot=False):
    """Init.
...
def make_last_layers(x, num_filters, out_filters):
    '''4 conv_block layers followed by a Conv2D_linear layer'''
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        if print_loss:
            loss = tf.Print(loss, [loss, xy_loss, wh_loss, confidence_loss, class_loss, K.sum(ignore_mask),K.sum(object_mask)], message='loss: ')
    return loss
